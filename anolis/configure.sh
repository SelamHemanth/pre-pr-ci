...
update_tests() {
	...
	echo "  7)  build_perf                 - Build perf tool"
	echo "  8)  boot_kernel_rpm            - Boot test (requires a VM)"
	echo "  9)  check_kapi                 - Check KAPI compatibility"
	echo "  10) check_dmesg                - Boot log of the booted kernel"
	echo ""
	...
	if [ "$TEST_SELECTION" == "all" ]; then
		...
		for test_num in "${SELECTED[@]}"; do
			case "${test_num// /}" in
				...
				7) TEST_BUILD_PERF="yes" ;;
				8) TEST_BOOT_KERNEL="yes" ; BOOT_SELECTED="yes" ;;
				9) TEST_CHECK_KAPI="yes" ;;
				10) TEST_CHECK_DMESG="yes" ;;
			esac
		done
	fi
	...
	rpms_dir="${LINUX_SRC_PATH}/anolis/outputs/rpmbuild/RPMS/$(arch)"
        ...
        if [[ "$BOOT_SELECTED" == "yes" || "$TEST_SELECTION" == "all" ]]; then
                ...
                if [[ "${BOOT_SELECTED:-}" == "yes" ]]; then
                        if [[ "${RPM_BUILD_SELECTED:-}" != "yes" ]]; then
                                # boot_kernel_rpm on its own has nothing to
                                # install unless a previous run left an RPM
                                # behind, so say so now rather than after the
                                # VM has been rebooted.
                                if [ ! -d "${rpms_dir}" ]; then
                                        fail "boot_kernel_rpm" "RPM directory not found: ${rpms_dir}. Select anck_rpm_build as well."
                                        exit 1
                                fi
                                ...
                                if [ -z "${kernel_rpm}" ]; then
                                        fail "boot_kernel_rpm" "No kernel RPM in ${rpms_dir}. Select anck_rpm_build as well."
                                        exit 1
                                fi
	...
	config_set "${CONFIG_FILE}" TEST_CHECK_DMESG "${TEST_CHECK_DMESG}"
	config_set "${CONFIG_FILE}" TEST_CHECK_KCONFIG    "${TEST_CHECK_KCONFIG}"
	config_set "${CONFIG_FILE}" TEST_BUILD_ALLYES     "${TEST_BUILD_ALLYES}"
	config_set "${CONFIG_FILE}" TEST_BUILD_ALLNO      "${TEST_BUILD_ALLNO}"
	config_set "${CONFIG_FILE}" TEST_BUILD_DEFCONFIG  "${TEST_BUILD_DEFCONFIG}"
	config_set "${CONFIG_FILE}" TEST_BUILD_DEBUG      "${TEST_BUILD_DEBUG}"
	config_set "${CONFIG_FILE}" TEST_RPM_BUILD        "${TEST_RPM_BUILD}"
	config_set "${CONFIG_FILE}" TEST_CHECK_KAPI       "${TEST_CHECK_KAPI}"
	config_set "${CONFIG_FILE}" TEST_BOOT_KERNEL      "${TEST_BOOT_KERNEL}"
	config_set "${CONFIG_FILE}" TEST_BUILD_PERF       "${TEST_BUILD_PERF}"
...
if ! torvalds_sync; then
	...
	echo -e "${YELLOW}the 'commit <sha> upstream.' line cannot be resolved or${NC}"
	echo -e "${YELLOW}checked until it is fixed.${NC}"
fi
...
# Test Configuration
RUN_TESTS="${RUN_TESTS}"
TEST_CHECK_DMESG="${TEST_CHECK_DMESG}"
